...
extension on double {
  bool isBetween(double num1, double num2) => num1 <= this && this <= num2;
}
...
class _TabViewport {
  ...
  bool contains(double x, double y, double totalLength) {
    ...
    switch (tabEdge) {
      case TabEdge.left:
        if (x <= tabExtent && y.isBetween(start, minEnd)) return true;
        break;
      case TabEdge.top:
        if (y <= tabExtent && x.isBetween(start, minEnd)) return true;
        break;
      case TabEdge.right:
...
/// A [TabContainer] that displays animated tabs with beautiful transitions.
class TabContainer extends StatefulWidget {
  ...
  /// Custom transition builder; if null, a beautiful default transition is applied.
...
class _TabContainerState extends State<TabContainer>
  ...
  TextStyle _calculateTextStyle(int index) {
    ...
    final fraction = _animationFraction(
      _progress,
      _controller.previousIndex,
      _controller.index,
    );
  ...
  double _calculateTextScale(int index) {
    ...
    final ratio = _selectedTextStyle.fontSize! / _unselectedTextStyle.fontSize!;
    if (index == _controller.index)
      return lerpDouble(1, ratio, fraction)!;
    else if (index == _controller.previousIndex)
      return lerpDouble(ratio, 1, fraction)!;
    else
      return 1.0;
  }
  ...
  Widget _getTab(int index) {
    ...
    if (widget.overrideTextProperties) return tab;
    return Transform(
  ...
  /// Improved default transition builder combining slide, scale, rotation, and fade.
...
/// A widget that provides focus support for the [TabContainer].
...
/// A widget that lays out the tab indicator and children.
...
// For brevity, the complete implementation of RenderTabFrame is included below.
// This version is nearly identical to your original code, with the option to add visual tweaks (like shadows) in _paint().
class RenderTabFrame extends RenderBox
  ...
  void _onTapDown(TapDownDetails details) {
    ...
    if (_tabViewport.contains(dx, dy, _tabMetrics.totalLength)) {
      double pos = dx;
      if (tabAxis == Axis.vertical) pos = dy;
  ...
  void performLayout() {
    ...
    bool tabViewportChanged =
        _prevTabViewport?.size != _tabViewport.size ||
    ...
    if (tabAxis == Axis.vertical) tabConstraints = tabConstraints.flipped;
  ...
  double computeMinIntrinsicWidth(double height) {
    ...
    if (tabAxis == Axis.vertical) return childMinIntrinsicWidth + tabExtent;
  ...
  double computeMaxIntrinsicWidth(double height) {
    ...
    if (tabAxis == Axis.vertical) return childMaxIntrinsicWidth + tabExtent;
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    if (tabAxis == Axis.vertical) return childMinIntrinsicHeight;
  ...
  double computeMaxIntrinsicHeight(double width) {
    ...
    if (tabAxis == Axis.vertical) return childMaxIntrinsicHeight;
      ...
      Path path =
          Path()
            ...
            ..quadraticBezierTo(
              width,
              height,
              width - borderRadius.bottomRight.x,
              height,
            )
            ...
            ..quadraticBezierTo(
              tabExtent,
              indicatorEnd,
              tabExtent - tabBorderRadius.bottomLeft.y,
              indicatorEnd,
            )
            ...
            ..quadraticBezierTo(
              tabExtent,
              0,
              tabExtent + borderRadius.topLeft.x,
              0,
            )
            ..close();
      if (tabEdge == TabEdge.right) {
  ...
  void _paint(PaintingContext context, Offset offset) {
    ...
    // Optionally, add a drop shadow for depth:
    canvas.drawShadow(
      _getPath(),
      const Color.fromARGB(0x40, 0, 0, 0),
      4.0,
      false,
    );
